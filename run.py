...
def open_link():
    ...
    for n in pages_list:
        url = 'https://www.ximalaya.com/revision/search?core=album&kw=' + name + '&page=' + str(
            n) + '&spellchecker=true&rows=20&condition=relation&device=iPhone'
...
def solve():
    ...
    for n in list1:
        url = 'http://180.153.255.6/mobile/v1/album/track/ts-1534855383505?albumId=' + albumId + '&device=android&isAsc=true&isQueryInvitationBrand=true&pageId=' + str(
            n) + '&pageSize=20&pre_page=0'
        html = requests.get(url)
        all = json.loads(html.text)
        data = all['data']['list']
        for a in data:
...
def get_time():
    ...
    :return:
    """
    r = s.get(time_api, headers=headers)
    return r.text
...
windows.title('喜马拉雅专辑下载器 v0.1 beta')
